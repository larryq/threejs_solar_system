import {
  q
} from "./chunk-YDGBZ5D2.js";
import {
  require_prop_types,
  shallow,
  subscribeWithSelector,
  useDrag
} from "./chunk-FOWMP7KC.js";
import {
  _extends,
  create,
  extend
} from "./chunk-V53A66ZX.js";
import {
  CustomShaderMaterial,
  require_glsl_token_descope,
  require_glsl_token_functions,
  require_glsl_token_string,
  require_string
} from "./chunk-XQMOCM2J.js";
import {
  Color,
  MathUtils,
  Matrix3,
...
var require_extend_shallow2 = __commonJS({
  "node_modules/extend-shallow/index.js"(exports, module) {
    ...
    module.exports = function extend2(o3) {
      if (!isObject3(o3)) {
        o3 = {};
      }
...
var Abstract = class {
  constructor(c3, props3, onParse) {
    this.uuid = MathUtils.generateUUID().replace(/-/g, "_");
    this.name = "LayerMaterial";
    this.mode = "normal";
    this.visible = true;
    const defaults = Object.getOwnPropertyNames(c3).filter((e3) => e3.startsWith("u_"));
    const uniforms = defaults.reduce((a3, v3) => {
      var _Object$getOwnPropert;
      let value = (_Object$getOwnPropert = Object.getOwnPropertyDescriptor(c3, v3)) == null ? void 0 : _Object$getOwnPropert.value;
    ...
    Object.keys(uniforms).map((key) => {
      ...
      properties[propName] = {
        set: (v3) => {
          this.uniforms[`u_${this.uuid}_${propName}`].value = getUniform(v3);
        },
        get: () => {
  ...
  buildShaders(constructor) {
    var _this$onParse;
    const shaders = Object.getOwnPropertyNames(constructor).filter((e3) => e3 === "fragmentShader" || e3 === "vertexShader").reduce((a3, v3) => {
      var _Object$getOwnPropert2;
      return {
        ...a3,
        [v3]: (_Object$getOwnPropert2 = Object.getOwnPropertyDescriptor(constructor, v3)) == null ? void 0 : _Object$getOwnPropert2.value
      };
    }, {});
    const tokens = {
      vert: (0, import_glsl_tokenizer.default)(shaders.vertexShader || ""),
      frag: (0, import_glsl_tokenizer.default)(shaders.fragmentShader || "")
    };
    const descoped = {
    ...
    const funcs = {
      vert: (0, import_glsl_token_functions.default)(descoped.vert),
      frag: (0, import_glsl_token_functions.default)(descoped.frag)
    };
    const mainIndex = {
      vert: funcs.vert.map((e3) => {
        return e3.name;
      }).indexOf("main"),
      frag: funcs.frag.map((e3) => {
        return e3.name;
      }).indexOf("main")
    };
    const variables = {
      vert: mainIndex.vert >= 0 ? (0, import_glsl_token_string.default)(descoped.vert.slice(0, funcs.vert[mainIndex.vert].outer[0])) : "",
      frag: mainIndex.frag >= 0 ? (0, import_glsl_token_string.default)(descoped.frag.slice(0, funcs.frag[mainIndex.frag].outer[0])) : ""
    };
    const funcBodies = {
  ...
  processFinal(shader, isVertex) {
    const s3 = shader.replace(/\sf_/gm, ` f_${this.uuid}_`).replace(/\(f_/gm, `(f_${this.uuid}_`);
    const returnValue = s3.match(/^.*return.*$/gm);
    let sReplaced = s3.replace(/^.*return.*$/gm, "");
  ...
  getBlendMode(b3, a3) {
    switch (this.mode) {
      default:
      case "normal":
        return `lamina_blend_alpha(${a3}, ${b3}, ${b3}.a)`;
      case "add":
        return `lamina_blend_add(${a3}, ${b3}, ${b3}.a)`;
      case "subtract":
        return `lamina_blend_subtract(${a3}, ${b3}, ${b3}.a)`;
      case "multiply":
        return `lamina_blend_multiply(${a3}, ${b3}, ${b3}.a)`;
      case "lighten":
        return `lamina_blend_lighten(${a3}, ${b3}, ${b3}.a)`;
      case "darken":
        return `lamina_blend_darken(${a3}, ${b3}, ${b3}.a)`;
      case "divide":
        return `lamina_blend_divide(${a3}, ${b3}, ${b3}.a)`;
      case "overlay":
        return `lamina_blend_overlay(${a3}, ${b3}, ${b3}.a)`;
      case "screen":
        return `lamina_blend_screen(${a3}, ${b3}, ${b3}.a)`;
      case "softlight":
        return `lamina_blend_softlight(${a3}, ${b3}, ${b3}.a)`;
      case "reflect":
        return `lamina_blend_reflect(${a3}, ${b3}, ${b3}.a)`;
      case "negation":
        return `lamina_blend_negation(${a3}, ${b3}, ${b3}.a)`;
    }
  }
  getSchema() {
  ...
  serialize() {
    ...
    nonUniformPropKeys = nonUniformPropKeys.filter((e3) => !["uuid", "uniforms", "schema", "fragmentShader", "vertexShader", "fragmentVariables", "vertexVariables", "attribs", "events", "__r3f", "onParse"].includes(e3));
    const nonUniformProps = {};
    nonUniformPropKeys.forEach((k3) => {
      nonUniformProps[k3] = this[k3];
    });
    const props3 = {};
...
var LayerMaterial$1 = class extends CustomShaderMaterial {
  ...
  genShaders() {
    ...
    let fragmentShader = "";
    let uniforms = {};
    this.layers.filter((l3) => l3.visible).forEach((l3) => {
      vertexVariables += l3.vertexVariables + "\n";
      fragmentVariables += l3.fragmentVariables + "\n";
      vertexShader += l3.vertexShader + "\n";
      fragmentShader += l3.fragmentShader + "\n";
      uniforms = {
        ...uniforms,
        ...l3.uniforms
      };
  ...
  set color(v3) {
    ...
    if ((_this$uniforms = this.uniforms) != null && (_this$uniforms$u_lami = _this$uniforms.u_lamina_color) != null && _this$uniforms$u_lami.value)
      this.uniforms.u_lamina_color.value = typeof v3 === "string" ? new Color(v3).convertSRGBToLinear() : v3;
  }
  get color() {
  ...
  set alpha(v3) {
    this.uniforms.u_lamina_alpha.value = v3;
  }
  get alpha() {
...
function getJSPropsFromLayer(layer) {
  ...
  entries.forEach(([key, val], idx) => {
    ...
    if (key.includes("color")) {
      const v3 = typeof val === "string" ? val : "#" + val.getHexString();
      props3 += `${key}: ${JSON.stringify(v3)},${eol}`;
    } else {
...
function serializedLayersToJS(layers, material) {
  ...
  const jsLayers = `${layers.map((l3) => {
    return `new ${l3.constructor}({
      ${getJSPropsFromLayer(l3)}
      })`;
  }).join(",\n		")}`;
  const js = `
  new LayerMaterial({
...
function DynamicLeva({
  ...
  useControls(name, () => {
    ...
    layers.forEach((layer, i3) => {
      const n3 = `${layer.label} ~${i3}`;
      o3[n3] = layer;
      o3[n3].onChange = () => setUpdate([`${name}.${n3}`, layer.label]);
    });
    return o3;
  }, {
    store
...
var DebugLayerMaterial = import_react26.default.forwardRef(({
  ...
  useControls({
    "Copy JSX": button(() => {
      const serialized = ref.current.layers.map((l3) => l3.serialize());
  ...
  import_react26.default.useEffect(() => {
    ...
    const schema3 = {};
    layers2.forEach((layer, i3) => {
      if (layer.getSchema)
        schema3[`${layer.name} ~${i3}`] = layer.getSchema();
    });
    setLayers(schema3);
  }, [children]);
  import_react26.default.useEffect(() => {
        ...
        (async () => {
          try {
            if (updatedData.value) {
              const t3 = await textureLoader.loadAsync(updatedData.value);
              layer[property] = t3;
              uniform.value = t3;
            } else {
  ...
  return import_react26.default.createElement(import_react26.default.Fragment, null, Object.entries(layers).map(([name, layers2], i3) => import_react26.default.createElement(DynamicLeva, {
    key: `${name} ~${i3}`,
    name,
    layers: layers2,
    store,
